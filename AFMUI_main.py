...
def read_data_on_fly(dataFile, scale):
    #print(data_File)
    ...
    z_scaled = np.round((scale/80)*4*z/z_max,3)
    ...
    #Y_convert_array = Y_convert_array/(np.max(Y_convert_array))
    X_scaled = np.array(interval_after_list)*np.max(Y_convert_array)#np.max(Y_to_plot)
    X_to_plot = X_scaled[0:n*m].reshape(n,m )
    
    Z_to_plot = z_scaled[0:n*m].reshape(n,m )
    
    #return X_to_plot, Y_to_plot, Z_to_plot
    return X_scaled, Y_convert_array, z_scaled, np.max(Y_convert_array)

def read_data_finish(dataFile):
    #print(data_File)
    df = pandas.read_csv(dataFile, sep=",", header= 0)
    data_array = np.array(df)
    X = data_array[:,0]
    ...
    Y_to_plot = np.array(Y_convert_list)[0:n*m].reshape(n,m )
    Y_convert_array  =  np.array(Y_convert_list)
    Y_convert_array = 80*Y_convert_array/(np.max(Y_convert_array))
...
class MainWindow(QWidget):
    def __init__(self, hub_client: HubClient, hub_monitor : HubMonitor) -> None:
        ...
        self.resolution = QGroupBox("Scan Resolution")
        self.resolution.setFont(QtGui.QFont("Sanserif", 12))
        self.reso_40 = QRadioButton("40 x 40")
        self.reso_40.setFont(QtGui.QFont("Sanserif", 12))
        self.reso_40.toggled.connect(self.radioButtonToggled)
        self.reso_40b = QRadioButton("40 x 40-B")
        self.reso_40b.setFont(QtGui.QFont("Sanserif", 12))
        self.reso_40b.toggled.connect(self.radioButtonToggled)
        self.reso_60 = QRadioButton("60 x 60")
        self.reso_60.setFont(QtGui.QFont("Sanserif", 12))
        self.reso_60.toggled.connect(self.radioButtonToggled)
        self.reso_60b = QRadioButton("60 x 60-B")
        self.reso_60b.setFont(QtGui.QFont("Sanserif", 12))
        self.reso_60b.toggled.connect(self.radioButtonToggled)
        self.reso_90 = QRadioButton("90 x 90")
        self.reso_90.toggled.connect(self.radioButtonToggled)
        self.reso_90b = QRadioButton("90 x 90B")
        self.reso_90b.setChecked(True)
        self.reso_90b.setFont(QtGui.QFont("Sanserif", 12))
        self.reso_90b.toggled.connect(self.radioButtonToggled)
        
        self.resolution_layout = QHBoxLayout()
        self.resolution_V_layout1 = QVBoxLayout()
        self.resolution_V_layout1.addWidget(self.reso_40)
        self.resolution_V_layout1.addWidget(self.reso_60)
        self.resolution_V_layout1.addWidget(self.reso_90)
        self.resolution_V_layout2 = QVBoxLayout()
        self.resolution_V_layout2.addWidget(self.reso_40b)
        self.resolution_V_layout2.addWidget(self.reso_60b)
        self.resolution_V_layout2.addWidget(self.reso_90b)
        ...
        self.operation_box = QGroupBox("Working Station")
        self.operation_box.setFont(QtGui.QFont("Sanserif", 15))
        ...
        self.graphical_box = QGroupBox("Output Station")
        self.graphical_box.setFont(QtGui.QFont("Sanserif", 15))
        ...
        self.pushButton = QPushButton() ##, clicked = lambda: self.plot())
        self.pushButton.clicked.connect(self.plot)
        
        self.pushButton.setObjectName("pushButton")
        self.pushButton.setText("Data On Fly")
        self.timer = QTimer()
        self.timer.timeout.connect(self.pushButton.click)
        
        self.NormalizedGraph = QPushButton()
        self.NormalizedGraph.setText("Nomalised Data")
        self.NormalizedGraph.clicked.connect(self.plot2)
            
        
        #pushButton.clicked.connect(self.plot())
        self.horizontalLayout2.addWidget(self.pushButton, 0)
        self.horizontalLayout2.addWidget(self.NormalizedGraph, 0)
        self.graphical_layout.addLayout(self.horizontalLayout2, 0)
        ...
        #self.questionBox.setWindowTitle("")
    ...
    def radioButtonToggled(self):
        ...
        #slot = None
        if self.reso_40.isChecked():
            slot = 4
            scale = 40
            print("You have selected " + self.reso_40.text() + "slot ", slot)
            
        elif self.reso_40b.isChecked():
            slot = 5
            scale = 40
            print("You have selected " + self.reso_40b.text() + "slot ", slot)
            
        elif self.reso_60.isChecked():
            scale = 60
            slot = 2
            print("You have selected " + self.reso_60.text() + "slot ", slot)
                
        elif self.reso_60b.isChecked():
            scale = 60
            slot = 3
            print("You have selected " + self.reso_60b.text() + "slot ", slot)
            
        elif self.reso_90.isChecked():
            scale = 90
            slot = 0
            print("You have selected " + self.reso_90.text() + "slot ", slot)
            
        elif self.reso_90b.isChecked():
            scale = 90
            slot = 1
            print("You have selected " + self.reso_90.text() + "slot ", slot)
    ...
    def run_program(self):
        ...
        slot, _ = self.radioButtonToggled()
    ...
    def plot(self):
        _, scale = self.radioButtonToggled()
        self.figure.clear()
        ...
        try:
            X_to_plot, Y_to_plot, Z_to_plot, set_limit = read_data_on_fly(data_file, scale)    
            ...
            try:
                ...
                self.errorBox.exec_()
        ax.set_ylim(0, set_limit)
        ax.set_xlim(0, set_limit)
    ...
    def plot2(self):
        ...
        ax.set_zlabel("Height")
        try:
            X_to_plot, Y_to_plot, Z_to_plot = read_data_finish(data_file)    
            ...
            try:
                ...
                self.errorBox.setIcon(QMessageBox.Critical)
                self.errorBox.setText(f"RuntimeError")
                self.errorBox.exec_()
        self.canvas.draw()
        self.timer.stop()
...
class WorkerThread(QThread):
    def run(self):
        while True:
            open_log(log_filename,data_file, dt_now )
            time.sleep(5)
